...
export default async function batchSemaphoreNoirProofs(
    ...
    for (let i = 0; i < proofs.length; i += 2) {
        const leafLayerPath = path.join(recursion, `leaf_layer_${i}`)
        mkdirSync(leafLayerPath, { recursive: true })
        ...
        await writeFile(`${leafLayerPath}/witness_${i}.gz`, witness)
        ...
        runBB([
            ...
            "-w",
            `${leafLayerPath}/witness_${i}.gz`,
            "-o",
            leafLayerPath,
            "--recursive"
        ])
        const proofFields = JSON.parse(readFileSync(`${leafLayerPath}/proof_fields.json`, "utf-8"))
    ...
    // The leafLayerProofs shouldn't work with this vk.
    ...
    while (currentLayerProofs.length > 1) {
        ...
        for (let i = 0; i < currentLayerProofs.length; i += 2) {
            const nodeLayerPath = path.join(recursion, `node_layer_${layer}_${i}`)
            mkdirSync(nodeLayerPath, { recursive: true })
            ...
            await writeFile(`${nodeLayerPath}/witness_nodes_${layer}_${i}.gz`, witness)
            ...
            runBB([
                ...
                "-w",
                `${nodeLayerPath}/witness_nodes_${layer}_${i}.gz`,
                "-o",
                nodeLayerPath,
                "--recursive"
            ])
            const proofFields = JSON.parse(readFileSync(`${nodeLayerPath}/proof_fields.json`, "utf-8"))
...
export async function batchSemaphoreNoirProofsPer4(
    proofs: SemaphoreNoirProof[],
    ...
    const tempDir = path.normalize(path.join("./", "semaphore_artifacts"))
    const recursion = path.join(tempDir, "recursion_4leaves")
    mkdirSync(recursion, { recursive: true })

    await mkdir(tempDir).catch((err) => {
        if (err.code !== "EEXIST") throw err
    })
    await writeFile(path.join(tempDir, `batch_4_leaves_circuit.json`), JSON.stringify(batchLeavesCircuit as any))
    await writeFile(path.join(tempDir, `batch_2_nodes_circuit.json`), JSON.stringify(batchNodesCircuit as any))

    const batchLeavesNoir = new Noir(batchLeavesCircuit)
    const batchNodesNoir = new Noir(batchNodesCircuit)
    const vkHash = `0x${"0".repeat(64)}`
    ...
    for (let i = 0; i < proofs.length; i += 4) {
        const leafLayerPath = path.join(recursion, `leaf_layer_${i}`)
        mkdirSync(leafLayerPath, { recursive: true })
        const chunk = proofs.slice(i, i + 4)
        if (chunk.length !== 4) throw new Error("Expected proof count divisible by 4")

        const sp = chunk.map((proof) => ({
            verification_key: semaphoreCircuitVk,
            proof: deflattenFields(proof.proofBytes),
            public_inputs: [
                hash(proof.scope).toString() as `0x${string}`,
                hash(proof.message).toString() as `0x${string}`,
                proof.merkleTreeRoot.toString() as `0x${string}`,
                proof.nullifier.toString() as `0x${string}`
            ],
            key_hash: vkHash
        }))

        const { witness } = await batchLeavesNoir.execute({ sp })
        await writeFile(`${leafLayerPath}/witness_4leaves_${i}.gz`, witness)
        ...
        runBB([
            ...
            "bytes_and_fields",
            "-b",
            `${tempDir}/batch_4_leaves_circuit.json`,
            "-w",
            `${leafLayerPath}/witness_4leaves_${i}.gz`,
            "-o",
            leafLayerPath,
            "--recursive"
        ])
